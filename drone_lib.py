...
def send_global_frame_velocities(device, velocity_x, velocity_y, velocity_z, duration=2):
    ...
    # NOTE: We turned both yaw and yaw rate bits OFF to avoid going in circles when using high velocity rates.
    #       Ardupilot version >4.0 is affected by this, since these bit flags are supported in those versions.
    ...
    # send command to vehicle @1Hz for specified duration
...
def send_body_frame_velocities(device, forward, right, velocity_z, duration=2):
    ...
    # NOTE: We turned both yaw and yaw rate bits OFF to avoid going in circles when using high velocity rates.
    #       Ardupilot version >4.0 is affected by this, since these bit flags are now supported in later versions.
